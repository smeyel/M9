...
nsCovRes = arrayfun(@(nx,ny) CalculateResultingCovariance([cam,CreateCamera([gX-nx;gY-ny],[nx;ny])], [gX;gY]), ...
    nX, nY);
...
nx = x(1);
ny = x(2);
nsCovRes = CalculateResultingCovariance([cam,CreateCamera([gX-nx;gY-ny],[nx;ny])], [gX;gY]);
nW = det(nsCovRes.Ci);
f = -nW;
